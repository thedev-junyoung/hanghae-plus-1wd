...
// ================== charge ==================
    ...
    // [정책 검증] 최대 잔액을 초과하는 금액을 충전할 경우 예외가 발생하는지 검증
    @Test
    void charge_잔액초과_충전_실패() {
        long amount = 1_000_000L;
        long currentBalance = PointPolicy.MAX_POINT_BALANCE;

        when(userPointTable.selectById(USER_ID))
                .thenReturn(new UserPoint(USER_ID, currentBalance, System.currentTimeMillis()));
